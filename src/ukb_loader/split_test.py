...
def test_fixed_split():
    ...
    samples = zarr.open_group(zarr_path, mode='r')['eid'][:]

    split_path = '/media/data1/ag3r/ukb/dataset/splits/fixed/'
    train, val, test = samples[:16], samples[16:18], samples[18:]
    splitter = FixedSplitter(zarr_path, split_path, train_samples=list(train.flatten()), val_samples=list(val.flatten()), test_samples=list(test.flatten()))
